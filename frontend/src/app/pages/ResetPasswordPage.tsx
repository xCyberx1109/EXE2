  ...
  if (!token) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent to-muted px-4 py-8">
        <div className="w-full max-w-md bg-card rounded-2xl shadow-xl overflow-hidden p-8 text-center">
          <div className="w-16 h-16 bg-destructive/10 rounded-full flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl text-destructive">!</span>
          </div>
          <h1 className="text-xl font-bold text-foreground mb-2">Link không hợp lệ</h1>
          <p className="text-muted-foreground mb-6">Thiếu token xác thực. Vui lòng kiểm tra lại link.</p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent to-muted px-4 py-8">
        <div className="w-full max-w-md bg-card rounded-2xl shadow-xl overflow-hidden p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6">Vui lòng đăng nhập lại.</p>
          <button
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent to-muted px-4 py-8">
      <div className="w-full max-w-md bg-card rounded-2xl shadow-xl overflow-hidden p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-foreground mb-1">Mật khẩu mới</label>
          ...
          >
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Đang xử lý...</> : <>Đặt lại mật khẩu</>}
          </button>
        </form>
