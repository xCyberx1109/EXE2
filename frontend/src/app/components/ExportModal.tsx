...
import { useInventoryItems, useBulkExportMutation } from '../api/hooks';
import { Loader2, Plus, X, ArrowUpFromLine } from 'lucide-react';
...
export function ExportModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  const [rows, setRows] = useState<ExportRow[]>([{ ingredientId: '', quantity: '' }]);
  ...
  const updateRow = (index: number, field: keyof ExportRow, value: string) => {
      ...
      <div ref={modalRef} className="bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-zinc-700 flex items-center justify-between">
          <h2 className="text-lg font-bold text-zinc-100 flex items-center gap-2">
            <ArrowUpFromLine className="size-5 text-orange-500" /> Xuất kho
          </h2>
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          ...
          <div>
                  ...
                  <div key={index} className="flex items-start gap-3">
                    <div className="flex-1">
                      <select
                        ...
                        className="w-full h-10 px-3 border border-zinc-700 rounded-lg bg-zinc-900 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 hover:border-zinc-600 transition-colors"
                      >
                    ...
                    <div className="w-28">
                      <input
                        ...
                        className="w-full h-10 px-3 border border-zinc-700 rounded-lg bg-zinc-900 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 hover:border-zinc-600 transition-colors"
                      />
                    </div>
                    <button
                      type="button"
            ...
            <button
              type="button"
              onClick={addRow}
              className="mt-3 flex items-center gap-1.5 text-sm text-orange-400 hover:text-orange-300 font-medium transition-colors"
            >
          ...
          {/* Lý do */}
          <div>
            <label className="block text-sm font-medium text-zinc-200 mb-3">Lý do xuất</label>
        ...
        <div className="px-6 py-4 border-t border-zinc-700 flex gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 h-10 px-4 bg-orange-600 text-white rounded-lg hover:bg-orange-500 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium flex items-center justify-center gap-2 transition-colors"
          >
            {loading ? <><Loader2 className="size-4 animate-spin" /> Đang xử lý...</> : 'Xác nhận xuất'}
          </button>
        </div>
      </div>
    </div>
