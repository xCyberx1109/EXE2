...
export const listPosTables = asyncHandler(async (req, res) => {
  ...
  console.log('[listPosTables] authType:', req.authType, 'contextId:', ctx?.id, 'branchId:', ctx?.branchId);
...
export const assignOrder = asyncHandler(async (req, res) => {
  ...
  const data = await tableService.assignOrder(req.params.id, orderId, req.user || req.posDevice);
...
export const updateTableStatus = asyncHandler(async (req, res) => {
  ...
  const data = await tableService.updateStatus(req.params.id, status, req.user || req.posDevice);
...
export const checkInReservation = asyncHandler(async (req, res) => {
  const data = await tableService.checkInReservation(req.params.id, req.user || req.posDevice);
